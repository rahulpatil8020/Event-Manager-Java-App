...
<body>
	...
	<div class="h-screen px-20">
		...
		<% }else{ %>
		<form action="update-event?id=<%= userId %>" method="post">
		<% }%>
		...
		<% if(event != null) { %>
			<div class="mt-5 mb-6 w-full px-3 md:mb-0 md:w-1/2">
				<label
					...
					id="name" name="name"
					type="text" value="<%= event.getName() %>" placeholder="Enter Event Name" required />
			</div>
			<div class="mb-6 w-full px-3 md:w-1/2">
				<label
					...
					id="location" value="<%= event.getLocation() %>" type="text" name="location" placeholder="Enter Event Location"
					required />
			</div>
			<div class="w-full px-3 md:w-1/2 mb-6">
				...
				<textarea
					class="block w-full appearance-none rounded border border-gray-200 bg-gray-200 py-3 px-4 leading-tight text-gray-700 focus:border-gray-500 focus:bg-white focus:outline-none"
					id="description" value="<%= event.getDescription() %>" name="description" placeholder="Enter the Description of the event"></textarea>
			</div>
			<div class="mx-3 w-max mb-6">
				<label
					...
					class="block w-full appearance-none rounded border border-gray-200 bg-gray-200 py-3 px-4 leading-tight text-gray-700 focus:border-gray-500 focus:bg-white focus:outline-none"
					id="date" name="date" value="<%= event.getDate() %>" type="datetime-local" required />
			</div>
			<%}else{ %>
							<div class="mt-5 mb-6 w-full px-3 md:mb-0 md:w-1/2">
				<label
